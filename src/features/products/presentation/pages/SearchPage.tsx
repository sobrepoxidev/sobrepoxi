...
function getSearchQuery(searchParams: { [key: string]: string | string[] | undefined } = {}) {
...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    title:
      locale === 'es'
        ? `Resultados de búsqueda${query ? ` | ${query}` : ''} | SobrePoxi`
        : `Search results${query ? ` | ${query}` : ''} | SobrePoxi`,
    description:
      locale === 'es'
          ...
          breadcrumb: {
            ...
            itemListElement: [
              ...
              },
              {
                '@type': 'ListItem',
                position: 3,
                name: `${locale === 'es' ? 'Resultados para' : 'Results for'}: ${query}`,
                item: `https://sobrepoxi.com/search?q=${encodeURIComponent(query)}`,
              },
            ],
          },
        })}
