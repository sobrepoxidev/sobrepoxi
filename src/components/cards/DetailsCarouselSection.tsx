...
/**
 * Componente específico para la sección de carrusel "Detalles artesanales"
 * Con estilos y comportamiento propios, incluyendo inversión de la lista en móvil
 */
const DetailsCarouselSection: React.FC = () => {
  ...
  const startIndex = 10;
  const endIndex = 20;
  const colors = ['bg-teal-500', 'bg-pink-600'];
  const title = locale === 'es' ? 'Detalles artesanales' : 'Artistic details';
  const mobileInvertList = true;
    ...
    <section className="py-6 px-4 relative">
      ...
      >
        {displayProducts.length > 0 ? (
          // Si mobileInvertList es true, se invierte el orden en móvil
          [...displayProducts]
            .map((product, index) => {
              // Selector de color alternando entre los colores proporcionados
              // Aplicamos efectiveIndex para invertir el orden visual
...
export default DetailsCarouselSection;
