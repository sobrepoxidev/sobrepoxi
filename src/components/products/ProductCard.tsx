...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      ...
      {/* Botón de favorito */}
      <button
        className="absolute top-1 right-1 z-50 text-black hover:text-[#b68b44] transition-colors"
        onClick={async (e) => {
          e.preventDefault();
      ...
      {/* Imagen del producto */}
      <Link href={`/product/${product.name}`} className="block h-48 sm:h-60 relative overflow-hidden">
        <div className="h-full w-full flex items-center justify-center bg-[#303030] p-0 overflow-hidden">
          <Image
            src={mainImageUrl}
            alt={product.name || ''}
            width={180}
      ...
      <div className="p-0.5  flex flex-col">
        <Link
          ...
          className="text-black font-medium hover:text-[#303030] transition-colors line-clamp-2 "
        >
        ...
        <div className="mt-auto">
          ...
          {/* Botones de acción */}
          <div className="flex flex-col space-y-1.5 mb-0.5 mt-auto">
            <Link
              href={`/product/${product.name}`}
            ...
            {inventory > 0 ? (
              <button
                ...
                className="flex items-center justify-center p-1 gap-0.5 rounded transition text-black border border-black hover:bg-black hover:text-white text-xs sm:text-sm"
                aria-label={locale === 'es' ? 'Agregar al carrito' : 'Add to cart'}
              >
                <ShoppingCart className="h-4 w-4 fill-current" />
                <span className="ml-1">{locale === 'es' ? 'Agregar' : 'Add'}</span>
              </button>
            ) : (
              <Link
