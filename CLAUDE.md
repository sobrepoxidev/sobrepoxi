...
**Migration status**: Phase 9 cleanup is mostly complete. Legacy directories are removed (`src/lib/`, `src/components/`, `src/i18n/`, `src/context/`, `src/utils/`, `src/types-db.ts`, `src/actions.ts`), and local gates pass with known `react-hooks/exhaustive-deps` warnings: `pnpm lint`, `pnpm typecheck`, `pnpm build`. Final sign-off still requires Vercel preview/manual smoke evidence.
<!-- SPECKIT END -->
